 ...
 */
    ...
    $sidebar_layout_config = kipso_get_option('default_sidebar_config', 'right-sidebar'); 
    $left_sidebar = kipso_get_option('default_left_sidebar', 'default_sidebar');
    $right_sidebar = kipso_get_option('default_right_sidebar', 'default_sidebar');
        ...
        <div class="row">
            <div class="content-page <?php echo esc_attr($main_content_config['class']); ?>"> 
                <div id="wp-content" class="wp-content">
                                                ...
                                                <div class="content-inner">
                                                    <div class="content-top entry-meta">
                                                       <?php if ( in_array( 'category', get_object_taxonomies( get_post_type() ) ) ){ ?>
                                                          <span class="cat-links"><?php echo get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'kipso' ) ); ?></span>
                                                          <span class="line"></span>
                                                       <?php } ?>
                                                       <?php kipso_posted_on(); ?>
                                                    </div> 
                                                    <h2 class="entry-title"><a href="<?php echo esc_url( get_permalink() ) ?>" rel="bookmark"><?php the_title() ?></a></h2>
                                                    <?php kipso_limit_words( 16, get_the_excerpt(), '' );?>
                                                </div>
                                                <?php the_tags( '<footer class="entry-meta"><span class="tag-links">', '', '</span></footer>' ); ?>
                                              <div class="read-more hidden"><a class="btn-theme" href="<?php echo esc_url( get_permalink() ) ?>"><?php echo esc_html__( 'Read more ', 'kipso' ) ?></a></div>
                    ...
                    <?php else: ?>
                        <div class="alert alert-danger"><?php esc_html_e( 'Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'kipso' ); ?></div>
                    <?php endif ?>
                    <div class="pagination">
                        <?php echo kipso_pagination(); ?>
                     </div>
                </div>
            </div>
