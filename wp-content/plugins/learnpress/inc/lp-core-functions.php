...
function learn_press_get_course_duration_support() {
	return apply_filters(
		...
		array(
			'minuto' => __( 'Minuto(s)', 'learnpress' ),
			'hora'   => __( 'Hora(s)', 'learnpress' ),
			'día'    => __( 'Día(s)', 'learnpress' ),
			'semana'   => __( 'Semana(s)', 'learnpress' )
		)
	);
}
...
function learn_press_seconds_to_weeks( $secs ) {
	...
	$result = '';
	if ( $weeks ) {
		$result .= $weeks . ' ' . _n( 'semana', 'semanas', $weeks, 'learnpress' ) . ' ';
	}

	if ( $days ) {
		$result .= $days . ' ' . _n( 'día', 'días', $days, 'learnpress' ) . ' ';
	}

	if ( ! $weeks ) {
		if ( $hours ) {
			$result .= $hours . ' ' . _n( 'hora', 'horas', $hours, 'learnpress' ) . ' ';

		}
		if ( $mins ) {
			if ( $message == false ) {
				$result .= $mins . ' ' . _n( 'minuto', 'minutos', $mins, 'learnpress' ) . ' ';
			} else {
